"""
OSCURS Python Daily Interpolation and Appending Reimplementation
==============================
...
Fortran-based workflow, and manages rolling yearly pressure files with
per-day updates. Logging is included to record all file creation and append
...
and fills any missing data via temporal interpolation. The final daily
output is written to a compressed NetCDF4 file, then inserted into or used
to initialize the yearly NetCDF file. Log entries provide traceability of
all corrections.
...
- `oscurs_pressure_YYYY.nc` : Yearly NetCDF file into which daily data is
  appended or merged.
- `oscurs_pressure_YYYY.log` : Plain-text log file recording overwrite,
  creation, and append events for the yearly file.
...
8. **Update Yearly File**:
   - If a yearly file does not exist, initialize it with the new daily file.
   - If a yearly file exists, call `upsert_daily_into_year()` to merge the
     daily values, overwriting overlaps and appending new data.
   - Write the merged dataset to disk atomically using a temporary file.

9. **Logging**:
   - Ensure a section header (`==== Daily Interpolation + Append ====`) exists
     in the log file.
   - Log all yearly file creations and append events with UTC timestamps
     and file names.
...
- Missing 00Z values are filled via weighted average of surrounding valid time
  steps.
- Daily NetCDF files are authoritative **only when differences exceed 0.01 mb**;
  otherwise, yearly values are preserved.
"""
...
time_hours = np.array(
    [
        date2num(
            num2date(t, "seconds since 1970-01-01T00:00:00Z"),
            "hours since 1948-01-01 00:00:00",
        )
        for t in time
    ]
)
...
pmsl = ncfile.createVariable(
    "pmsl", "f4", ("time", "y", "x"), zlib=True, complevel=4, fill_value=-9999.0
)
...
# Define the output directory for yearly NetCDF files (use current working directory)
YEARLY_OUTDIR = os.getcwd()

# Extract year value from earlier parsing of input_date
year = start_year

# File paths for daily input, yearly output, and logs
daily_path = outfile

# Path to yearly NetCDF file
yearly_path = os.path.join(YEARLY_OUTDIR, f"oscurs_pressure_{year}.nc")

# Log file associated with yearly file
log_file = yearly_path.replace(".nc", ".log")

# Temporary path for atomic write replacement
temp_path = yearly_path + ".tmp"

# Create a timestamp string in UTC for logging events
timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")

# Ensure that a section header is present in the log file
# If the log file does not exist, create it and write the header once
if not os.path.exists(log_file):
    with open(log_file, "w") as log:
        log.write("==== Daily Interpolation + Append ====\n")

# Case 1: If the yearly NetCDF file does not yet exist
if not os.path.exists(yearly_path):
    print(f"Creating new yearly file: {os.path.basename(yearly_path)}")
    with open_minimal(daily_path) as ds_daily:
        ds_daily.load()  # Load dataset into memory
        ds_daily.to_netcdf(
            yearly_path, mode="w", format="NETCDF4", engine="netcdf4"
        )  # Write daily dataset as the new yearly file

    # Log file creation event
    with open(log_file, "a") as log:
        log.write(
            f"{timestamp} | Created yearly file from {os.path.basename(daily_path)}\n"
        )

# Case 2: If the yearly file already exists
else:
    # Open both the existing yearly file and the new daily dataset
    with open_minimal(yearly_path) as ds_year, open_minimal(
        daily_path
    ) as ds_daily:
        # Merge daily into yearly (daily values overwrite any overlapping times)
        ds_combined = upsert_daily_into_year(ds_year, ds_daily, log_file)
        ds_combined.load()  # Ensure combined dataset is fully in memory
        ds_combined.to_netcdf(
            temp_path, mode="w", format="NETCDF4", engine="netcdf4"
        )  # Write to temporary file

    # Atomically move the temporary file to replace the old yearly file
    shutil.move(temp_path, yearly_path)

    # Log the append event
    with open(log_file, "a") as log:
        log.write(f"{timestamp} | Appended {os.path.basename(daily_path)}\n")

    # Print confirmation to the console
    print(
        f"Appended {os.path.basename(daily_path)} into {os.path.basename(yearly_path)}"
    )

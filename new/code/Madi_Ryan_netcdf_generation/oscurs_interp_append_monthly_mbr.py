"""
OSCURS Python Interpolation and Appending Reimplementation
==============================
...
per-month updates. Logging is included to record all file creation and
overwrite operations for reproducibility.
...
and fills any missing data via temporal interpolation. The final monthly
...
- `oscurs_pressure_YYYY.nc` : Yearly NetCDF file into which monthly data is
  appended or merged.
- `oscurs_pressure_YYYY.log` : Plain-text log file recording overwrite and
  creation events for the yearly file.
...
7. **Save Monthly Output**:
   - Write interpolated fields and metadata to a compressed CF-1.6-compliant
     NetCDF file.

8. **Update Yearly File**:
   - If a yearly file does not exist, initialize it with the new monthly file.
   - If a yearly file exists, call `upsert_monthly_into_year()` to merge the
     monthly values, overwriting overlaps and appending new data.
   - Write the merged dataset to disk atomically using a temporary file.

9. **Logging**:
   - Append section headers (`==== Monthly Final Correction ====`) and timestamped
     entries to a log file for each yearly dataset.
   - Log entries record whether a yearly file was created, monthly values were
     applied, and when appends were completed.
...
- Missing 00Z values are filled via weighted average of surrounding valid time
  steps.
- Monthly NetCDF files are authoritative and replace any overlapping entries
  in the yearly file.
"""
...
# Extract the target year and build file paths
year = start_year
monthly_path = outfile

# Yearly NetCDF file path
yearly_path = os.path.join(YEARLY_OUTDIR, f"oscurs_pressure_{year}.nc")

# Log file corresponding to yearly file
log_file = yearly_path.replace(".nc", ".log")

# Temporary path used before replacing yearly file
temp_path = yearly_path + ".tmp"

# Current timestamp (UTC) for log entries
timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")

# Case 1: Yearly file does not yet exist
if not os.path.exists(yearly_path):
    # Create new yearly file with the contents of the monthly file
    print(f"Creating new yearly file: {os.path.basename(yearly_path)}")
    with open_minimal(monthly_path) as ds_monthly:
        ds_monthly.load()
    ...
    with open(log_file, "a") as log:
        log.write("==== Monthly Final Correction ====\n")
        log.write(
            f"{timestamp} | Created yearly file from {os.path.basename(monthly_path)}\n\n"
        )

# Case 2: Yearly file already exists
else:
    # Open existing yearly and new monthly datasets
    with open_minimal(yearly_path) as ds_year, open_minimal(
        monthly_path
    ) as ds_monthly:
        # Start logging the overwrite process
        with open(log_file, "a") as log:
            log.write("==== Monthly Final Correction ====\n")
            log.write(
                f"{timestamp} | Applying overwrites from {os.path.basename(monthly_path)}\n"
            )

        # Merge monthly into yearly (monthly values overwrite overlaps)
        ds_combined = upsert_monthly_into_year(ds_year, ds_monthly, log_file)
        ds_combined.load()  # Ensure data is in memory before saving
        ds_combined.to_netcdf(
            temp_path, mode="w", format="NETCDF4", engine="netcdf4"
        )

    # Atomically replace old yearly file with the new merged file
    shutil.move(temp_path, yearly_path)
    print(
        f"Appended {os.path.basename(monthly_path)} into {os.path.basename(yearly_path)}"
    )

    # Log completion of append process
    with open(log_file, "a") as log:
        log.write(
            f"{timestamp} | Finished append of {os.path.basename(monthly_path)}\n\n"
        )
